...
class CommentVotePersistenceListener implements EventSubscriberInterface {
    ...
    public function avoidIncorrectVoting(VotePersistEvent $event) {
        ...
        if (!$this->context->isGranted(AuthenticatedVoter::IS_AUTHENTICATED_REMEMBERED)) {
            ...
            throw new HttpException(409, 'Avoid voting if user is not authenticated');
        }
        /** @var $vote SignedVoteInterface */
        $vote = $event->getVote();
        /** @var $user User */
        ...
        if ($vote->getVoter() !== $user) {
            $this->stopPersistence($event);
            throw new HttpException(409, 'Attempt to vote for different user');
        }
        if ($vote->getComment()->getAuthor() === $user) {
            $this->stopPersistence($event);
            throw new HttpException(409, 'You cannot vote on your own comment.');
        }
        ...
        if ($existingVote) {
            $this->stopPersistence($event);
            throw new HttpException(409, 'You cannot vote more than once.');
        }
        ...
        if (null !== $existingVote && $existingVote->getValue() !== $vote->getValue()) {
        }
    }
